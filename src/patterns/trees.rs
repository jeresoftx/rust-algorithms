...
///
/// Pattern: post-order DFS returning validity, bounds and sum.
/// Time: O(n). Space: O(height).
pub fn max_sum_bst(root: TreeLink) -> i32 {
    fn visit(node: &TreeLink, best: &mut i32) -> (bool, i32, i32, i32) {
        let Some(node) = node else {
            return (true, i32::MAX, i32::MIN, 0);
        };
        let node = node.borrow();
        let (left_ok, left_min, left_max, left_sum) = visit(&node.left, best);
        let (right_ok, right_min, right_max, right_sum) = visit(&node.right, best);
        if left_ok && right_ok && left_max < node.val && node.val < right_min {
            let sum = left_sum + node.val + right_sum;
            *best = (*best).max(sum);
            (true, left_min.min(node.val), right_max.max(node.val), sum)
        } else {
